        ...
        <?php if ($isCopy): ?>
            <div class="card card-selection" data-url="<?= Utils::urlTo('site/copy/' . $type . '/' . base64_encode($f) . '?p=' . $vFolder['link']) ?>">
                <div class="row card-body px-3 py-2 align-items-center">
                    <div class="col-auto" style="width: calc(100% - 180px);"><h5 class="text-nowrap mb-0 py-1 text-truncate"><i class="<?= $vFolder['bi_icon'] ?>"></i> <?= FileSystem::convertWin($vFolder['encFile']) ?></h5></div>
                    <div class="d-nones d-lg-table-cells" style="width: 180px;"><?= $vFolder['modification_date'] ?></div>
                </div>
            </div>
        <?php else: ?>
            <div class="card card-selection" data-card-id="card-folder-<?= $kFolder ?>" data-url="<?= Utils::urlTo('site/index/' . $vFolder['link']) ?>">
                <div class="row card-body px-3 py-2 align-items-center">
                    <div class="col-auto" style="width: calc(100% - 500px);"><h5 class="text-nowrap mb-0 py-1 text-truncate"><i class="<?= $vFolder['bi_icon'] ?>"></i> <?= FileSystem::convertWin($vFolder['encFile']) ?></h5></div>
                    ...
                    <div class="text-end" style="width: 220px;">
                        <p class="mb-0">
                            <?php if (App::$session->getPermission('cCompress')): ?>
