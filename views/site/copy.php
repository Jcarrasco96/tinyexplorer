...
<div class="container">
    ...
    <div class="d-flex align-items-center justify-content-between mb-2">
        <?php if ($parent !== false): ?>
            <a class="btn btn-bd-primary me-2" href="<?= Utils::urlTo('site/copy/' .  $type . '/' . base64_encode($f) . '?p=' . base64_encode($parent)) ?>"><i class="bi bi-arrow-left"></i></a>
            <?= BreadcrumbCopy::run(['path' => $p, 'file' => $f, 'type' => $type]) ?>
        <?php else: ?>
            <a class="btn btn-bd-primary me-2" href="<?= Utils::urlTo('site/index') ?>"><i class="bi bi-house-fill"></i></a>
        <?php endif; ?>
...
<script>
    ...
    $(document).on("click", "#btn-copy, #btn-move", function (event) {
        ...
        $.ajax({
            ...
            success: function(data) {
                /** @var {string} data.status */
                window.location.assign("<?= Utils::urlTo('site/index/' . base64_encode($p)) ?>");
            },
            error: function(jqXHR, textStatus) {
                nerror(jqXHR.responseJSON.message);
            },
